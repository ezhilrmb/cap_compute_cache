   ...
   , m_min_dummy_inst(0)
{
   ...
   Sim()->getHooksManager()->registerHook(HookType::HOOK_MAGIC_MARKER, PerformanceModel::hookProcessAppMagic, (UInt64)this, HooksManager::ORDER_NOTIFY_PRE);
}
void PerformanceModel::processAppMagic(UInt64 argument) {
	...
	if (Sim()->getCfg()->getBool("general/microbench_run")) {
      ...
      m_instruction_queue.pop();
			...
			if(	(m_instruction_queue.size() == 0) 
					&& is_last_loop 
					&& (getInstructionCount() < target_instruction_cnt)
				) {
				//put in a dummy instruction
				if(!dummy_inst) {
					OperandList dummy_list;
					dummy_list.push_back(Operand(Operand::REG, 76, Operand::READ,
																													"", true));
  				dummy_inst 	= new GenericInstruction(dummy_list);
  				dummy_inst->setAddress(400);
      		...
      		currentMicroOp->setMemBarrier(true);
  				currentMicroOp->addSourceRegister((xed_reg_enum_t)76, "");
