...
int avoid_index, int avoid_index2)
{
   ...
   const UInt32 index = getReplacementIndex(cntlr, avoid_index, avoid_index2);
   assert(index < m_associativity);
	 assert(index != avoid_index);
	 assert(index != avoid_index2);
