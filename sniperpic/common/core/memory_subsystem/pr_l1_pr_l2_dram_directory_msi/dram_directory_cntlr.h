         ...
         void updateShmemPerf(ShmemMsg *shmem_msg, ShmemPerf::shmem_times_type_t reason = ShmemPerf::UNKNOWN)
         {
            shmem_msg->getPerf()->updateTime(getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD), reason);
				 ...
				 //#ifdef PIC_ENABLE_OPERATIONS
   				std::unordered_map<IntPtr, IntPtr> picSrc1ToSrc2;
   				std::unordered_map<IntPtr, IntPtr> picSrc2ToSrc1;
         	void processPicCopyFromL2Cache(ShmemReq* shmem_req, 
																				Byte* cached_data_buf = NULL);
         	void processPicCmpFromL2Cache(ShmemReq* shmem_req, 
																				Byte* cached_data_buf = NULL);
          bool picRetrieveData(core_id_t receiver, 
						IntPtr address1, IntPtr address2, 
						Byte* cached_data_buf, ShmemMsg *orig_shmem_msgi, 
						bool is_dest_copy);
				 //#endif
				 #ifdef NUCA_INCLUSIVE
          void processDirectoryEntryEvictionReq(IntPtr address, 
																					core_id_t receiver);
   				std::set<IntPtr> nucaEvictions;
				 #endif
