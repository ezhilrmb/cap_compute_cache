...
namespace PrL1PrL2DramDirectoryMSI
{
   ...
   ShmemMsg::ShmemMsg(ShmemMsg* shmem_msg) :
      ...
      m_perf(shmem_msg->getPerf())
			//#ifdef PIC_ENABLE_OPERATIONS
			,m_other_address(shmem_msg->m_other_address)
			,m_other_address2(shmem_msg->m_other_address2)
			//#endif
   {}
         ...
         case FLUSH_DATA_REP:
            ...
            return (1 + sizeof(IntPtr) + m_data_length);
				 ...
				 //#ifdef PIC_ENABLE_OPERATIONS
					...
					case VPIC_COPY_REP:
					case VPIC_CMP_REP:
					case VPIC_SEARCH_REP:
            return (1 + 2 * sizeof(IntPtr));
					case VPIC_SEARCH_REQ:
            return (9 + 2 * sizeof(IntPtr)); //8byte key data
				 //#endif
