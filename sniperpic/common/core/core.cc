...
void Core::psuedo_warmup() {
	if (Sim()->getCfg()->getBool("general/microbench_run")) {
		if(getId() == 0) {
	 		assert(Sim()->getInstrumentationMode() == InstMode::CACHE_ONLY);
			getMemoryManager()->create_microbench_instructions();
		}
	}
}

void Core::psuedo_iterate() {
	if (Sim()->getCfg()->getBool("general/microbench_run")) {
		if(getId() == 0) {
			while(getMemoryManager()->more_microbench_loops()) {
				getMemoryManager()->create_microbench_instructions();
				getPerformanceModel()->psuedo_iterate(false, 0);
			}
			//last loop done, wait till target is reached
			printf("\nNeed: %lu instructions\n", getMemoryManager()->microbench_target_ins_cnt());
			getMemoryManager()->create_microbench_instructions();
			getPerformanceModel()->psuedo_iterate(true, 
				getMemoryManager()->microbench_target_ins_cnt());
		}
	}
}
