...
def main(jobid, resultsdir, outputfile, powertype = 'dynamic', config = None, no_graph = False, partial = None, print_stack = True, return_data = False, bank_map_policy = 'all_ways', no_mcpat_run = False, in_cache_frac = -1.0, add_implicit_accesses = False):
...
def edit_XML(statsobj, stats, cfg):
  ...
  if int(cfg['perf_model/cache/levels']) >= 3:
    ...
    if 'nuca-cache.pic_ops_copy' in stats:
      ...
      stats['L%d.pic_ops_clmult'%l] = stats['nuca-cache.pic_ops_clmult']
      if in_cache_frac != -1:
        ...
        stats['L%d.pic_ops_in_bank_clmult_'%l + bank_map_policy] 	= [(x * in_cache_frac)  for x in stats['L%d.pic_ops_clmult'%l]]
      else:
        ...
        stats['L%d.pic_ops_in_bank_clmult_'%l + bank_map_policy] = stats['L%d.pic_ops_clmult'%l]
      ...
      if template[i][1]:
        ...
        if len(template[i][1]) == 1:
          ...
          if template[i][1][0]=="total_cycles":
            ...
            template[i][0] = template[i][0] % (long(stats['L1-D.stores'][core]) + long('L1-D.pic_vops_copy' in stats and stats['L1-D.pic_vops_copy'][core] or 0) + + long('L1-D.pic_vops_clmult' in stats and stats['L1-D.pic_vops_copy'][core] or 0))
          ...
          elif template[i][1][0]=="dcache.pic_clmult_ops":
            template[i][0] = template[i][0] % long(('L1-D.pic_ops_clmult' in stats and stats['L1-D.pic_ops_clmult'][core] or 0))
          elif template[i][1][0]=="dcache.pic_clmult_ops_in_bank":
            if in_cache_frac != -1:
             template[i][0] = template[i][0] % long(('L1-D.pic_ops_clmult' in stats and ((in_cache_frac * stats['L1-D.pic_ops_clmult'][core]) )) or 0)
            else:
             template[i][0] = template[i][0] % long(\
									('L1-D.pic_ops_in_bank_clmult_' + bank_map_policy in stats and (stats['L1-D.pic_ops_in_bank_clmult_' + bank_map_policy][core]) or 0))
          elif template[i][1][0]=="dcache.pic_clmult_tag_access":
            template[i][0] = template[i][0] % long(\
									('L1-D.pic_ops_tag_access_clmult' in stats and stats['L1-D.pic_ops_tag_access_clmult'][core] or 0))
          elif template[i][1][0]=="dcache.pic_clmult_read_miss":
            template[i][0] = template[i][0] % long(\
									('L1-D.pic_ops_read_miss_clmult' in stats and stats['L1-D.pic_ops_read_miss_clmult'][core] or 0))
          elif template[i][1][0]=="dcache.pic_clmult_write_miss":
            template[i][0] = template[i][0] % long(\
									('L1-D.pic_ops_write_miss_clmult' in stats and stats['L1-D.pic_ops_write_miss_clmult'][core] or 0))
          elif template[i][1][0]=="function_calls":
          ...
          elif template[i][1][0]=="L2.pic_clmult_ops":
            template[i][0] = template[i][0] % long( \
															('L2.pic_ops_clmult' in stats and sum([ stats['L2.pic_ops_clmult'][c] for c in cores_l2s]) or 0))
          elif template[i][1][0]=="L2.pic_clmult_ops_in_bank":
            if in_cache_frac != -1:
             template[i][0] = template[i][0] % long( 'L2.pic_ops_clmult' in stats and \
		         ((sum([(in_cache_frac * stats['L2.pic_ops_clmult'][c]) for c in cores_l2s ]))) or 0)
            else:
             template[i][0] = template[i][0] % long ( \
             ('L2.pic_ops_in_bank_clmult_' + bank_map_policy in stats and sum([ stats['L2.pic_ops_in_bank_clmult_' + bank_map_policy ][c] for c in cores_l2s ]) or 0) or 0)
          elif template[i][1][0]=="L2.pic_clmult_tag_access":
            template[i][0] = template[i][0] % long ( \
                       ('L2.pic_ops_tag_access_clmult' in stats and sum([ stats['L2.pic_ops_tag_access_clmult'][c] for c in cores_l2s ]) or 0))
          elif template[i][1][0]=="L2.pic_clmult_read_miss":
            template[i][0] = template[i][0] % long ( \
											('L2.pic_ops_read_miss_clmult' in stats and sum([ stats['L2.pic_ops_read_miss_clmult'][c] for c in cores_l2s ]) or 0))
          elif template[i][1][0]=="L2.pic_clmult_write_miss":
            template[i][0] = template[i][0] % long ( \
												('L2.pic_ops_write_miss_clmult' in stats and sum([ stats['L2.pic_ops_write_miss_clmult'][c] for c in cores_l2s ]) or 0))
          ...
          elif template[i][1][0]=="L3.pic_clmult_ops":
            template[i][0] = template[i][0] % ('L3.pic_ops_clmult' in stats and sum([ stats['L3.pic_ops_clmult'][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3.pic_clmult_ops_in_bank":
            template[i][0] = template[i][0] % ('L3.pic_ops_in_bank_clmult_' + bank_map_policy in stats and sum([ stats['L3.pic_ops_in_bank_clmult_' + bank_map_policy][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3.pic_clmult_tag_access":
            template[i][0] = template[i][0] % ('L3.pic_ops_tag_access_clmult' in stats and sum([ stats['L3.pic_ops_tag_access_clmult'][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3.pic_clmult_read_miss":
            template[i][0] = template[i][0] % ('L3.pic_ops_read_miss_clmult' in stats and sum([ stats['L3.pic_ops_read_miss_clmult'][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3.pic_clmult_write_miss":
            template[i][0] = template[i][0] % ('L3.pic_ops_write_miss_clmult' in stats and sum([ stats['L3.pic_ops_write_miss_clmult'][c] for c in cores_l3s ]) or 0)
...
#       template.append(["\t\t\t\t<param name=\"buffer_sizes\" value=\"%i, 16, %i, %i\"/>",["icache_buffer","comb"]]) #mcpat will crash for some different sizes of 2nd parameter
    ...
    template.append(["\t\t\t\t<stat name=\"pic_clmult_ops\" value=\"%i\"/>",["dcache.pic_clmult_ops","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_ops_in_bank\" value=\"%i\"/>",["dcache.pic_clmult_ops_in_bank","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_tag_access\" value=\"%i\"/>",["dcache.pic_clmult_tag_access","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_read_miss\" value=\"%i\"/>",["dcache.pic_clmult_read_miss","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_write_miss\" value=\"%i\"/>",["dcache.pic_clmult_write_miss","stat",iCount]])
    template.append(["\t\t\t</component>",""])
...
#------------------------------------------------------------------------------------------------------------------------
  for iCount in range(num_l2s):
    ...
    template.append(["\t\t\t\t<stat name=\"pic_clmult_ops\" value=\"%i\"/>",["L2.pic_clmult_ops","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_ops_in_bank\" value=\"%i\"/>",["L2.pic_clmult_ops_in_bank","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_tag_access\" value=\"%i\"/>",["L2.pic_clmult_tag_access","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_read_miss\" value=\"%i\"/>",["L2.pic_clmult_read_miss","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_write_miss\" value=\"%i\"/>",["L2.pic_clmult_write_miss","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
...
#------------------------------------------------------------------------------------------------------------------------
  for iCount in range(num_l3s):
    ...
    template.append(["\t\t\t\t<stat name=\"pic_clmult_ops\" value=\"%i\"/>",["L3.pic_clmult_ops","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_ops_in_bank\" value=\"%i\"/>",["L3.pic_clmult_ops_in_bank","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_tag_access\" value=\"%i\"/>",["L3.pic_clmult_tag_access","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_read_miss\" value=\"%i\"/>",["L3.pic_clmult_read_miss","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"pic_clmult_write_miss\" value=\"%i\"/>",["L3.pic_clmult_write_miss","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
...
if __name__ == '__main__':
  ...
  try:
    opts, args = getopt.getopt(sys.argv[1:], "hij:t:c:d:o:p", [ 'no-graph', 'no-text', 'partial=' , 'no-mcpat-run', 'in-cache-frac='])
  ...
  main(jobid = jobid, resultsdir = resultsdir, powertype = powertype, config = config, outputfile = outputfile, no_graph = no_graph, print_stack = not no_text, partial = partial, bank_map_policy = bank_map_policy, no_mcpat_run = no_mcpat_run, in_cache_frac = in_cache_frac, add_implicit_accesses = add_implicit_accesses)
