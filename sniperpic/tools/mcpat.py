...
def main(jobid, resultsdir, outputfile, powertype = 'dynamic', config = None, no_graph = False, partial = None, print_stack = True, return_data = False, bank_map_policy = 'all_ways'):
      ...
      if template[i][1]:
        ...
        if len(template[i][1]) == 1:
          ...
          if template[i][1][0]=="total_cycles":
            ...
            template[i][0] = template[i][0] % int('L1-D.pic_ops_copy' in stats and stats['L1-D.pic_ops_copy'][core] or 0)
          elif template[i][1][0]=="dcache.pic_copy_ops_in_bank":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_in_bank_copy_' + bank_map_policy in stats and stats['L1-D.pic_ops_in_bank_copy_' + bank_map_policy][core] or 0)
          elif template[i][1][0]=="dcache.pic_copy_tag_access":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_tag_access_copy' in stats and stats['L1-D.pic_ops_tag_access_copy'][core] or 0)
          elif template[i][1][0]=="dcache.pic_copy_read_miss":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_read_miss_copy' in stats and stats['L1-D.pic_ops_read_miss_copy'][core] or 0)
          elif template[i][1][0]=="dcache.pic_copy_write_miss":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_write_miss_copy' in stats and stats['L1-D.pic_ops_write_miss_copy'][core] or 0)
          elif template[i][1][0]=="dcache.pic_cmp_ops":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_cmp' in stats and stats['L1-D.pic_ops_cmp'][core] or 0)
          elif template[i][1][0]=="dcache.pic_cmp_ops_in_bank":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_in_bank_cmp_' + bank_map_policy in stats and stats['L1-D.pic_ops_in_bank_cmp_' + bank_map_policy][core] or 0)
          elif template[i][1][0]=="dcache.pic_cmp_tag_access":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_tag_access_cmp' in stats and stats['L1-D.pic_ops_tag_access_cmp'][core] or 0)
          elif template[i][1][0]=="dcache.pic_cmp_read_miss":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_read_miss_cmp' in stats and stats['L1-D.pic_ops_read_miss_cmp'][core] or 0)
          elif template[i][1][0]=="dcache.pic_cmp_write_miss":
            template[i][0] = template[i][0] % int('L1-D.pic_ops_write_miss_cmp' in stats and stats['L1-D.pic_ops_write_miss_cmp'][core] or 0)
          elif template[i][1][0]=="function_calls":
          ...
          # L2 caches
            ...
            template[i][0] = template[i][0] % ( 'L2.pic_ops_cmp' in stats and sum([ stats['L2.pic_ops_cmp'][c] for c in cores_l2s]) or 0)
          elif template[i][1][0]=="L2.pic_cmp_ops_in_bank":
            template[i][0] = template[i][0] % ( 'L2.pic_ops_in_bank_cmp_' + bank_map_policy in stats and sum([ stats['L2.pic_ops_in_bank_cmp_' + bank_map_policy ][c] for c in cores_l2s ]) or 0)
          elif template[i][1][0]=="L2.pic_cmp_tag_access":
            template[i][0] = template[i][0] % ( 'L2.pic_ops_tag_access_cmp' in stats and sum([ stats['L2.pic_ops_tag_access_cmp'][c] for c in cores_l2s ]) or 0)
          elif template[i][1][0]=="L2.pic_cmp_read_miss":
            template[i][0] = template[i][0] % ( 'L2.pic_ops_read_miss_cmp' in stats and sum([ stats['L2.pic_ops_read_miss_cmp'][c] for c in cores_l2s ]) or 0)
          elif template[i][1][0]=="L2.pic_cmp_write_miss":
            template[i][0] = template[i][0] % ( 'L2.pic_ops_write_miss_cmp' in stats and sum([ stats['L2.pic_ops_write_miss_cmp'][c] for c in cores_l2s ]) or 0)
...
if __name__ == '__main__':
  ...
  try:
    opts, args = getopt.getopt(sys.argv[1:], "hj:t:c:d:o:p:", [ 'no-graph', 'no-text', 'partial=' ])
  ...
  main(jobid = jobid, resultsdir = resultsdir, powertype = powertype, config = config, outputfile = outputfile, no_graph = no_graph, print_stack = not no_text, partial = partial, bank_map_policy = bank_map_policy)
